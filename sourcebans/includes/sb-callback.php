					...
					if(!defined('IN_HOME')) {
						...
						foreach($players AS $player) {
							$objResponse->addScript('var e = document.getElementById("playerlist_'.$sid.'");
													...
													tr.id = "player_s'.$sid.'p'.$player["index"].'";
														...
														var td = tr.insertCell("-1");
															...
															var txt = document.createTextNode("'.str_replace('"', '\"',  iconv('cp1251', 'utf-8', $player["name"])).'");
															td.appendChild(txt);
														// Score TD
														var td = tr.insertCell("-1");
															td.className = "listtable_1";
							...
							if($userbank->HasAccess(ADMIN_OWNER|ADMIN_ADD_BAN)) {
								$objResponse->addScript('AddContextMenu("#player_s'.$sid.'p'.$player["index"].'", "contextmenu", true, "Что делаем?", [
														
														{name: "Бан", callback: function(){window.location = "index.php?p=admin&c=bans&action=pasteBan&sid='.$sid.'&pName='.str_replace('"', '\"',  iconv('cp1251', 'utf-8', $player["name"])).'"}},
														{separator: true},
														{name: "Скриншот", callback: function(){GetSS("'.$sid.'", "'.$player["index"].'")}}
														]);');
							}
							$playercount++;
						}
...
function PasteBan($sid, $name, $type=0)
{
	...
	foreach($matches[2] AS $match) {
		if(iconv('cp1251', 'utf-8', $match) == $name ) {
			$found = true;
			$index = $i;
			break;
		}
	...
	$parts = array("0", "0", "0");
	if(strpos($steam, "STEAM_") === 0)
	{
		$parts = explode(":", substr($steam, 6));
	}
	...
	$steam = $friendid["friend_id"];
		...
		$ip = $ip[0];
		$objResponse->addScript("$('nickname').value = '" . iconv('cp1251', 'utf-8', addslashes($name)) . "'");
		if($type==1)
...
$guid = $steam;
	...
	$subid = $GLOBALS['db']->Insert_ID();
		...
		$stats_db->exec("UPDATE ss SET banned = 1 where guid = '$guid'");
		$stats_db = NULL;
	...
	$kickit = isset($GLOBALS['config']['config.enablekickit']) && $GLOBALS['config']['config.enablekickit'] == "1";
		...
		$objResponse->addScript('ShowKickBox("'.((int)$type==0?$steam:$ip).'", "'.(int)$type.'", "'.$reason.'");');
...
function GetSS($sid, $guid)
{
	...
	{
		$objResponse->redirect("index.php?p=login&m=no_access", 0);
		$log = new CSystemLog("w", "Hacking Attempt", $username . " tried to SS ingame message to '".$guid."' but doesnt have access.");
		return $objResponse;
	}
	$sid = (int)$sid;
	require INCLUDES_PATH.'/CServerRcon.php';
	...
	{
		$GLOBALS['db']->Execute("UPDATE ".DB_PREFIX."_servers SET rcon = '' WHERE sid = '".$sid."';");
		$objResponse->addScript("ShowBox('Error', 'Wrong RCON password!', 'red', '', true);");
		return $objResponse;
	}
	$ret = $r->sendCommand("getss ".$guid);
  new CSystemLog("m", "Успех", "Запрошен скриншот игрока номер " . $guid . " на сервере " . $data['ip'] . ":" . $data['port']);
	$objResponse->addScript("ShowBox('Успех','Скриншот успешно запрошен!', 'green', '', true);$('dialog-control').setStyle('display', 'block');");
	return $objResponse;
}
