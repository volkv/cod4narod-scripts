...
define('SB_WP_URL','http://178.57.220.145/sourcebans/');       				//URL of SourceBans Site
...
$perm_bans_query = $sourcebans_db->query("SELECT DISTINCT(authid) from sb_bans where length = 0");
while ($row = $perm_bans_query->fetch()) {
	echo SteamtoGUID($row["authid"]) . "</br>";
	}