...
//<![CDATA[
window.onload = function() {xajax_LoadServers('-{$check}-', '-{$type}-', "-{$reason}-");}
var srvcount = 0;
function set_counter(count)
{
	srvcount += count;
	if(srvcount==-{$total}- || count=='-1') {
		parent.document.getElementById('dialog-control').innerHTML = "<font color=\"green\" style=\"font-size: 12px;\"><b>Поиск окончен</b></font>"+parent.document.getElementById('dialog-control').innerHTML;
...
<h3 style="font-size: 12px;">Ищем говнюка по всем серверам...</h3>
<table border="0">
-{foreach from=$servers item=serv}-
<tr>
	<td><div id="srvip_-{$serv.num}-"><font size="1">-{$serv.ip}-:-{$serv.port}-</font></div></td>
	<td>
		<div id="srv_-{$serv.num}-"><font size="1">Ждем...</font></div>
	</td>
</tr>
-{/foreach}-
</table>
