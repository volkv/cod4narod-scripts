...
$theme->assign('plname', $_GET['plname']);

if (!empty($_GET['pldemo'])) {


    $theme->assign('pldemo', $_GET['pldemo']);

    $filename = md5(time() . rand(0, 1000));

    file_put_contents(SB_DEMOS . "/" . $filename, fopen($_SERVER['DOCUMENT_ROOT'] . "/screenshots/files/" . $_GET['pldemo'], 'r'));

    $message = "<script>demo('" . $filename . "','" . $_GET['pldemo'] . "');</script>";

    $theme->assign("message", $message);
}

$theme->assign('plguid', $_GET['plguid']);
$theme->assign('plip', $_GET['plip']);
