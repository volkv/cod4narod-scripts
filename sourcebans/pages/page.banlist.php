...
elseif(!isset($_GET['advSearch']))
{
	...
	"SELECT bid ban_id, BA.type, BA.ip ban_ip, BA.authid, BA.name player_name, created ban_created, ends ban_ends, length ban_length, reason ban_reason, BA.ureason unban_reason, BA.aid, AD.gid AS gid, adminIp, BA.sid ban_server, country ban_country, RemovedOn, RemovedBy, RemoveType row_type,
			...
			CAST(MID(BA.authid, 9, 1) AS UNSIGNED) + CAST('76561197960265728' AS UNSIGNED) + CAST(MID(BA.authid, 11, 10) * 2 AS UNSIGNED) AS community_id,
			(SELECT count(*) FROM ".DB_PREFIX."_demos as DM WHERE DM.demtype='B' and DM.demid = BA.bid) as demo_count,
			(SELECT count(*) FROM ".DB_PREFIX."_bans as BH WHERE  BH.authid = BA.authid ) as history_count
	   FROM ".DB_PREFIX."_bans AS BA
  LEFT JOIN ".DB_PREFIX."_servers AS SE ON SE.sid = BA.sid
  LEFT JOIN ".DB_PREFIX."_mods AS MO on SE.modid = MO.mid
  LEFT JOIN ".DB_PREFIX."_admins AS AD ON BA.aid = AD.aid
...
//	}
	...
	$data['edit_link'] = CreateLinkR('<img src="images/edit.gif" border="0" alt="" style="vertical-align:middle" /> Редактировать бан',"index.php?p=admin&c=bans&o=edit".$pagelink."&id=".$res->fields['ban_id']."&key=".$_SESSION['banlist_postkey']);
    ...
    if($res->fields['history_count'] > 1)
        ...
        $data['prevoff_link'] = "Других банов нет";
	...
	if ($res->fields['demo_count'] == 0)
	{
		...
		$data['demo_quick'] = 'N/A';
		$data['demo_link'] = CreateLinkR('<img src="images/demo.gif" border="0" alt="" style="vertical-align:middle" /> Доказательств нет',"#");
	}
	else
	{
		$data['demo_available'] = true;
		$data['demo_quick'] = CreateLinkR('Demo',"getdemo.php?type=B&id=".$data['ban_id']);
		$data['demo_link'] = CreateLinkR('<img src="images/demo.gif" border="0" alt="" style="vertical-align:middle" /> Доказательства преступления',"getdemo.php?type=B&id=".$data['ban_id']);
	}
...
if ($page > 1)
{
	if(isset($_GET['c']) && $_GET['c'] == "bans")
		$prev = CreateLinkR('<img border="0" alt="prev" src="images/left.gif" style="vertical-align:middle;" /> сюда',"javascript:void(0);", "", "_self", false, $prev);
	else
		$prev = CreateLinkR('<img border="0" alt="prev" src="images/left.gif" style="vertical-align:middle;" /> сюда',"index.php?p=banlist&page=".($page-1).(isset($_GET['searchText']) > 0?"&searchText=".$_GET['searchText']:'' . $advSearchString));
}
else
{
	$prev = "";
...
if ($BansEnd < $BanCount)
{
	if(isset($_GET['c']) && $_GET['c'] == "bans")
	{
		if(!isset($nxt))
			$nxt = "";
			$next = CreateLinkR('туда <img border="0" alt="next" src="images/right.gif" style="vertical-align:middle;" />',"javascript:void(0);", "", "_self", false, $nxt);
	}
	else
		$next = CreateLinkR('туда <img border="0" alt="next" src="images/right.gif" style="vertical-align:middle;" />',"index.php?p=banlist&page=".($page+1).(isset($_GET['searchText']) ?"&searchText=".$_GET['searchText']:'' . $advSearchString));
}
else
	$next = "";

//=================[ Start Layout ]==================================
$ban_nav = 'показано&nbsp;'.$BansStart.'&nbsp;-&nbsp;'.$BansEnd.'&nbsp;из&nbsp;'.$BanCount.'&nbsp;банов';
