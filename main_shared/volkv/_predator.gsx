...
init()
{
	...
	{
		self iPrintLnBold( self volkv\_common::getLangString("PREDATOR_NOT_AVAILABLE") );
		return false;
	}
...
setup()
{
	...
	level volkv\_languages::iPrintSmall("PREDATOR_INCOMING", "PLAYER", self.name);
...
infoHUD()
{
	...
	self.info[ 0 ] setText(self volkv\_common::getLangString("PREDATOR_HINT") );
	...
	self.info[ 1 ] setText(self volkv\_common::getLangString("TIME_LEFT") );
	...
	self.info[ 3 ] setText( self volkv\_common::getLangString("PREDATOR_LEFT","AGM", self.predatorAmmoLeft) );
	...
	while( isDefined( level.flyingPlane ) && isDefined( self.info ) )
	{
		...
		{
			self.info[ 0 ] setText(self volkv\_common::getLangString("PREDATOR_HINT2")  );
