...
init()
{
	...
	{
		self iPrintLnBold( self volkv\_common::getLangString("AC180_NOT") );
		return false;
	}
...
setup()
{
	...
	self thread volkv\_languages::iPrintBigTeams("CALL_AC180", "PLAYER", self.name);
...
infoHUD()
{
	...
	self.info[ 0 ] setText( self volkv\_common::getLangString("AC180_HINT"));
			...
			for( i = 0; i < ents.size; i++ )
			{
				if ( !ents[ i ].isPlayer || isAlive( ents[ i ].entity ) )
				{
					...
					if( isPlayer( ents[ i ].entity ) )
						ents[ i ].entity.sWeaponForKillcam = "ac180_105mm";
			...
			for( n = 0; n < ents.size; n++ )
			{
				if ( !ents[ n ].isPlayer || isAlive( ents[ n ].entity ) )
				{
					...
					if( isPlayer( ents[ n ].entity ) )
						ents[ n ].entity.sWeaponForKillcam = "ac180_40mm";
		...
		case 2:
			...
			for( n = 0; n < ents.size; n++ )
			{
				if ( !ents[ n ].isPlayer || isAlive( ents[ n ].entity ) )
				{
					...
					if( isPlayer( ents[ n ].entity ) )
						ents[ n ].entity.sWeaponForKillcam = "ac180_25mm";
