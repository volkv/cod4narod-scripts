...
notifyAllBig( string )
{
	...
	for( i = 0; i < players.size; i++ )
	{
		...
		player iPrintLnBold("^1" + string );
	}
}

notifyTeamBig( string )
{
...
restoreHP()
{
	...
	if( level.hardcoreMode )
		self.maxhealth = 30 + self.HPbonus;
		
	else if( level.oldschool )
		self.maxhealth = 200 + self.HPbonus;

	else
		self.maxhealth = 100 + self.HPbonus;
