	...
	while( isDefined( ent ) )
	{
		...
		if( getTime() - ent.fireTime > 500 )
		{
			...
			if( collision[ "surfacetype" ] != "default" && collision[ "fraction" ] < 1 ) 
			{
				...
				thread playSoundinSpace( "artillery_impact", collision[ "position" ] );
...
selectLocation()
{
	...
	{
		self iPrintLnBold(self volkv\_common::getLangString("ARTILLERY_NOT_AVAILABLE"));
		return false;
	}
