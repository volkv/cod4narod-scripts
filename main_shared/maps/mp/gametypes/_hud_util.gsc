...
showPerk_side( index, perk, ypos )
{
	...
	if ( !isdefined( self.perkicon_side[ index ] ) )
	{
		...
		self.perkicon_side[ index ] = icon;
	}

	icon = self.perkicon_side[ index ];
...
showPerk( index, perk, hor )
{
	...
	if ( !isdefined( self.perkicon[ index ] ) )
	{
		...
		xpos = hor - (10 + iconsize) * (2 - index);

		icon = createIcon( "white", iconsize, iconsize );
		icon setPoint( "CENTER", undefined, xpos, ypos );
		icon.archived = false;
		icon.foreground = true;
...
hidePerk( index, fadetime, hideTextOnly )
{
	...
	if (isDefined(self.perkicon) && isDefined(self.perkicon[ index ] ))
		self.perkicon[ index ] thread volkv\_common::fadeOut(1);

	if (isDefined(self.perkicon_side) && isDefined(self.perkicon_side[ index ] ))
		self.perkicon_side[ index ] thread volkv\_common::fadeOut(1);
