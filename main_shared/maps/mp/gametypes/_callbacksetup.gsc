...
{
	self endon("disconnect");
	// self thread volkv\_anticheat::PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, timeOffset);
...
{
	self endon("disconnect");
	// self thread volkv\_anticheat::PlayerKilled(eInflictor, eAttacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, timeOffset, deathAnimDuration);
...
CodeCallback_PlayerView() {
	...
	//iPrintLnBold("move");
	//iPrintLnBold(volkv\_common::getAngleDistance(self.currentframe[1],self.lastframe[1]));
...
KoneDetection(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, timeOffset) {
	if( !isDefined(eAttacker) || !isPlayer(eAttacker) || sWeapon == "saw_bipod_crouch_mp" || eAttacker.currentframe[1] - eAttacker.lastframe[1] > 100 || sMeansOfDeath == "MOD_MELEE" )
		return;
	angle = volkv\_common::getAngleDistance(eAttacker.lastframe[0],eAttacker.currentframe[0]);
	...
	avg = volkv\_common::getAverageValue(eAttacker.pers["aimangle"]);
	if(avg>1)
		volkv\_common::devPrint(eAttacker + " may got kone");
	else 
		if(eAttacker volkv\_common::isDev())
...
SetDefaultCallbacks()
{
	...
	level.callbackScriptCommand = volkv\cmds::Callback_ScriptCommand;
}
...
CodeCallback_PlayerSayCmd(text) {
	if(text == "menu")
		...
		maybe = undefined;
		if(("" + int(player)) == player && isDefined(volkv\_common::getPlayerByNum(int(player))))
			maybe = volkv\_common::getPlayerByNum(int(player));
		else {
		...
		if(isDefined(maybe)) {
			...
			volkv\_common::log("private_chat.log","FROM: "+self.name+"("+getSubStr(self getGuid(),24,32)+") TO: "+maybe.name+"("+getSubStr(maybe getGuid(),24,32)+") MSG: " + getSubStr(text,4 + player.size,text.size),"append");
		}
		else 
			exec("tell " + self getEntityNumber() + " ^5Couldn't find player ^7'" +player + "'");
	}
	else {
		volkv\_common::log("commands.log",self.name + " (" + getSubStr(self getGuid(),24,32) +"): " + text,"append");
