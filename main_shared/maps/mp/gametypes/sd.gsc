...
timer(time) {
	...
	text.label = volkv\_common::getLangString("RESPAWN_IN");
...
Recorder() {
	...
	for(;;) {
		...
		if(self UseButtonPressed()) {
			...
			if(ifstart) {
				volkv\_common::log("locations_"+level.script+".log","else if(level.script == "+wtf+""+getDvar("mapname")+""+wtf+") {","append");
				ifstart = false;				
			}
			if(mode == "axis")
				volkv\_common::log("locations_"+level.script+".log","	" +start+"\n	level.trainingAreaSpawns["+wtf+"axis"+wtf+"][i]["+wtf+"origin"+wtf+"] = "+self.origin+";\n	level.trainingAreaSpawns["+wtf+"axis"+wtf+"][i]["+wtf+"angles"+wtf+"] = "+self GetPlayerAngles()+";","append");
			else if(mode == "allies")
				...
				volkv\_common::log("locations_"+level.script+".log","	level.trainingAreaFire[level.trainingAreaFire.size] = "+self.origin+";","append");
			start = "i++;";
		}
		else if(self MeleeButtonPressed()) {
			iPrintLnBold("New sector");
			volkv\_common::log("locations_"+level.script+".log","	//--------------------------------","append");
		}
			...
			volkv\_common::log("locations_"+level.script+".log","	//Recorder Mode: " +mode,"append");
			start = "i=0;";
		}
