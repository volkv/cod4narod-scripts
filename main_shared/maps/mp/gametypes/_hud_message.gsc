...
onPlayerConnect()
{
	for(;;)
	{
		...
		player.doingNotify_lite = false;
		player.notifyQueue = [];
		player.notifyQueue_lite = [];	
...
initNotifyMessage_lite()
{
	if(isDefined(self.notifyTitle_lite)) return;
	...
	self.notifyTitle_lite = createFontString( font, titleSize );
	self.notifyTitle_lite setPoint( point, undefined, xOffset, yOffset );
	self.notifyTitle_lite.hideWhenInMenu = true;
	self.notifyTitle_lite.archived = false;
	self.notifyTitle_lite.color = (0,1,0);
	self.notifyTitle_lite.alpha = 0;

	self.notifyText_lite = createFontString( font, textSize );
	self.notifyText_lite setParent( self.notifyTitle_lite );
	self.notifyText_lite setPoint( point, relativePoint, 0, 10 );
	self.notifyText_lite.hideWhenInMenu = true;
	self.notifyText_lite.archived = false;
	self.notifyText_lite.color = (1,1,0);
	self.notifyText_lite.alpha = 0;
...
oldNotifyMessage_lite( titleText, notifyText )
{
	notifyData = spawnstruct();
...
notifyMessage_lite( notifyData )
{
	...
	if ( !self.doingNotify_lite )
	{
		self thread showNotifyMessage_lite( notifyData );
		return;
	}
	self.notifyQueue_lite[ self.notifyQueue_lite.size ] = notifyData;
...
showNotifyMessage( notifyData )
{
	...
	if ( isDefined( notifyData.titleText ) )
	{
		...
		//	self.notifyTitle setPulseFX( 100, int(duration*1000), 1000 );
			self.notifyTitle fadeOverTime( 0.3 );
	...
	if ( isDefined( notifyData.notifyText ) )
	{
		...
		//	self.notifyText setPulseFX( 100, int(duration*1000), 1000 );
			self.notifyText fadeOverTime( 0.3 );
	...
	if ( isDefined( notifyData.iconName ) )
	{
		...
		self.notifyIcon fadeOverTime( 0.3 );
		self.notifyIcon.alpha = 0;
...
showNotifyMessage_lite( notifyData )
{
	self endon("disconnect");
	
	self.doingNotify_lite = true;

	self initNotifyMessage_lite();

	waitRequireVisibility( 0 );

		duration = 2.0;
	
	 self thread resetOnCancel_lite();
	
	anchorElem = self.notifyTitle_lite;
	...
	if ( isDefined( notifyData.titleText ) )
	{
			...
			if ( isDefined( notifyData.titleLabel ) )
				self.notifyTitle_lite.label = notifyData.titleLabel;
			else
				self.notifyTitle_lite.label = &"";

			if ( isDefined( notifyData.titleLabel ) && !isDefined( notifyData.titleIsString ) )
				self.notifyTitle_lite setValue( notifyData.titleText );
			else
				self.notifyTitle_lite setText( notifyData.titleText );
		//	self.notifyTitle_lite setPulseFX( 100, int(duration*1000), 1000 );
			self.notifyTitle_lite fadeOverTime( 0.3 );
			self.notifyTitle_lite.alpha = 1;
	...
	if ( isDefined( notifyData.notifyText ) )
	{
			...
			if ( isDefined( notifyData.textLabel ) )
				self.notifyText_lite.label = notifyData.textLabel;
			else
				self.notifyText_lite.label = &"";
	
			if ( isDefined( notifyData.textLabel ) && !isDefined( notifyData.textIsString ) )
				self.notifyText_lite setValue( notifyData.notifyText );
			else
				self.notifyText_lite setText( notifyData.notifyText );

			self.notifyText_lite fadeOverTime( 0.3 );
			self.notifyText_lite.alpha = 1;
			anchorElem = self.notifyText_lite;
...
wait .5;
	self notify ( "notifyMessageDone_lite" );
	self.doingNotify_lite = false;
	if ( self.notifyQueue_lite.size > 0 )
	{
		nextNotifyData = self.notifyQueue_lite[0];
		
		newQueue = [];
		for ( i = 1; i < self.notifyQueue_lite.size; i++ )
			self.notifyQueue_lite[i-1] = self.notifyQueue_lite[i];
		self.notifyQueue_lite[i-1] = undefined;
		
		self thread showNotifyMessage_lite( nextNotifyData );
	}
	else 
		self thread resetNotify_lite();
...
resetOnCancel_lite()
{
	self notify ( "resetOnCancel_lite" );
	self endon ( "resetOnCancel_lite" );
	self endon ( "notifyMessageDone_lite" );
	self endon ( "disconnect" );
...
resetNotify_lite()
{
	if(isDefined(self.notifyTitle_lite))	
		self.notifyTitle_lite destroy();
	if(isDefined(self.notifyText_lite))	
		self.notifyText_lite destroy();
