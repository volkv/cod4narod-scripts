...
init()
{
	...
	priority++;
	level.hardpointItems["heli_mp"] = priority;
	priority++;
	level.hardpointItems["ac180_mp"] = priority;
	priority++;
	...
	level.hardpointHints["ac180_mp"] = "Press [{+actionslot 4}] for AC130 GUNSHIP";
	...
	level.hardpointHints["ac180_mp_not_available"] = "AC130 GUNSHIP not available";
	...
	level.hardpointInforms["ac180_mp"] = "mp_killstreak_jet";
...
giveHardpointItemForStreak()
{
	...
	if (streak == 1 ) {
		...
		self showPerk(0,"specialty_fastreload",80);
		...
		self showPerk(0,"specialty_detectexplosive",80);

		self.detectExplosives = true;
		self maps\mp\gametypes\_weapons::setupBombSquad();
		self thread maps\mp\gametypes\_globallogic::hidePerksAfterTime( 1.0 );
		self thread maps\mp\gametypes\_globallogic::hidePerksOnDeath();
	}
		...
		self showPerk(0,"specialty_bulletaccuracy",80);
		...
		self showPerk(0,"specialty_holdbreath",80);
		...
		self showPerk(0,"specialty_longersprint",80);
		self thread maps\mp\gametypes\_globallogic::hidePerksAfterTime( 1.0 );
		self thread maps\mp\gametypes\_globallogic::hidePerksOnDeath();
		...
		self showPerk(0,"specialty_gpsjammer",80);
		self thread maps\mp\gametypes\_globallogic::hidePerksAfterTime( 1.0 );
		self thread maps\mp\gametypes\_globallogic::hidePerksOnDeath();
		...
		self showPerk(0, "specialty_fraggrenade", 80 );
		self thread maps\mp\gametypes\_globallogic::hidePerksAfterTime( 1.0 );
		self thread maps\mp\gametypes\_globallogic::hidePerksOnDeath();
		...
		self showPerk(0,"specialty_quieter",80);
		self thread maps\mp\gametypes\_globallogic::hidePerksAfterTime( 1.0 );
		self thread maps\mp\gametypes\_globallogic::hidePerksOnDeath();
	}
		...
		self giveHardpoint( "heli_mp", streak );
		...
		self showPerk(0,"specialty_rof",60);
		self showPerk( 1, "specialty_extraammo", 60 );
		self thread maps\mp\gametypes\_globallogic::hidePerksAfterTime( 1.0 );
		self thread maps\mp\gametypes\_globallogic::hidePerksOnDeath();
		...
		self giveHardpoint( "ac180_mp", streak );
		...
		self showPerk(0,"specialty_grenadepulldeath",80);
		self thread maps\mp\gametypes\_globallogic::hidePerksAfterTime( 1.0 );
		self thread maps\mp\gametypes\_globallogic::hidePerksOnDeath();
...
streakNotify( streakVal )
{
	...
	self maps\mp\gametypes\_hud_message::notifyMessage_lite( notifyData );
...
giveHardpoint( hardpointType, streak )
{
	...
	if (isDefined(self) && self maps\mp\gametypes\_hardpoints::giveHardpointItem( hardpointType ) )
	{
...
hardpointNotify( hardpointType, streakVal )
{
	...
	notifyData.titleLabel = &"MP_KILLSTREAK_N";
	notifyData.titleText = streakVal;
	if (hardpointType == "predator_mp" || hardpointType == "strafe_mp" || hardpointType == "carepackage_mp" || hardpointType == "artillery_mp" || hardpointType=="orbital_mp" || hardpointType =="nuke_mp" || hardpointType =="ac180_mp" || hardpointType == "heli_mp")
...
giveHardpointItem( hardpointType )
{
	...
	if (hardpointType == "predator_mp" || hardpointType == "carepackage_mp" || hardpointType == "artillery_mp" || hardpointType == "orbital_mp" || hardpointType == "nuke_mp" || hardpointType == "ac180_mp") {
		hardpointType = "radar_mp";
	}
	if (hardpointType =="heli_mp" || hardpointType == "strafe_mp") {
...

	{
		...
		case "ac180_mp":
			if ( self triggerHardpoint( currentWeapon ) )
			{
				...
				if (self.pers["hardPointItem"] != "predator_mp" && self.pers["hardPointItem"] != "strafe_mp" && self.pers["hardPointItem"] != "carepackage_mp"  && self.pers["hardPointItem"] != "artillery_mp" && self.pers["hardPointItem"] != "orbital_mp" && self.pers["hardPointItem"] != "nuke_mp" && self.pers["hardPointItem"] != "ac180_mp" && self.pers["hardPointItem"] != "heli_mp")
...
triggerHardPoint( hardpointType )
{
	...
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "predator_mp")
	{
		...
		result =  self thread volkv\_predator::init();
	...
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "artillery_mp")
	{
		...
		result = self volkv\artillery::selectLocation();

		if ( !isDefined( result ) || !result )
			return false;

	}
	else if ( hardpointType == "helicopter_mp" && self.pers["hardPointItem"] == "strafe_mp")
	{
		...
		if ( isDefined( level.strafeInProgress ))
		{
			...
			self thread volkv\_strafe_run::StrafeRun();
		}
	}
	...
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "ac180_mp")
	{



		result = self volkv\ac180::init();
	...
	else if ( hardpointType == "helicopter_mp" && self.pers["hardPointItem"] == "heli_mp")
	{




		result = self volkv\heli::init();
	...
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "carepackage_mp")
	{
		...
		if (self volkv\_carepackage::canCallPackage())
...
UAVAcquiredPrintAndSound( team, otherteam, callingPlayer, numseconds )
{
		...
		for ( i = 0; i < level.players.size; i++ )
		{
			...
			{
				if ( playerteam == team )
					player iprintln(&"MP_WAR_RADAR_ACQUIRED", "^2" + callingPlayer.name, numseconds );
				else if ( playerteam == otherteam )
					player iprintln(&"MP_WAR_RADAR_ACQUIRED", "^1" + callingPlayer.name, numseconds );
			}
		}
