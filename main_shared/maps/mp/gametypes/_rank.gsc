...
init()
{
	...
	registerScoreInfo( "melee", 500 );
	registerScoreInfo( "explosive", 50 );
	registerScoreInfo( "impact", 333 );
	registerScoreInfo( "assist", 50 );
	registerScoreInfo( "suicide", -50 );
	registerScoreInfo( "teamkill", -100 );

	
	registerScoreInfo( "win", 1 );
	registerScoreInfo( "loss", 0.5 );
	registerScoreInfo( "tie", 0.75 );
	registerScoreInfo( "capture", 1000 );
	registerScoreInfo( "assault", 500 );
	registerScoreInfo( "assault_assist", 250 );
	registerScoreInfo( "defend", 500 );
	registerScoreInfo( "defend_assist", 250 );

	registerScoreInfo( "plant", 1000 );
	registerScoreInfo( "defuse", 1000 );
...
initScoreHud( fullUpdate )
{
	...
	if( !level.newScoreUpdate )
	{
		...
		self.hud_rankscroreupdate = sniper::newScoreUpdate();
}
...
XPFlyIn() {
	...
	if(!isDefined(value) || !value)
		return;
	xpflyin = volkv\_common::addTextHud( self, RandomIntRange(90,100), RandomIntRange(-15,5), 1, "left", "bottom", "left", "bottom", 1.6, 1 );
...
XPBar() {
	...
	self.xpbar.label = self volkv\_common::getLangString("EARNED_XP");
...
notifyAdmins(name,status){
	...
	for(i = 0; i < players.size; i++)	{
		...
		switch (status) {
			case "admin":
			status_notify = players[i] volkv\_common::getLangString("STATUS_ADMIN");
			break;
			case "vip":
			status_notify = players[i] volkv\_common::getLangString("STATUS_VIP");
			break;
			case "vipadmin":
			status_notify = players[i] volkv\_common::getLangString("STATUS_VIPADMIN");
			break;
		}
		players[i] iprintln(players[i] volkv\_common::getLangString("STATUS_NOTIFY","NAME",name,"STATUS",status_notify));
	}
...
onPlayerConnect()
{
	...
	rankId = player getRankForXp(player.pers["killz"]);
	
	player.pers["rankxp"] = player.pers["killz"];
	player.pers["rank"] = rankId;
	
	player maps\mp\gametypes\_persistence::statSet( "minxp", int(level.rankTable[0][2]) );
	player maps\mp\gametypes\_persistence::statSet( "maxxp", int(level.rankTable[0][7]) );
	
		if( !isDefined( player.pers[ "meleekills" ] ) )
		player.pers[ "meleekills" ] = 0;
	
			if( !isDefined( player.pers[ "grenadekills" ] ) )
		player.pers[ "grenadekills" ] = 0;
	
		player.pers[ "youVSfoe" ] = [];
		player.pers[ "youVSfoe" ][ "killedBy" ] = [];
		player.pers[ "youVSfoe" ][ "killed" ] = [];
...
//if (isDefined(player.pers["status"]) && isDefined(player.name)) {
//	if (player.pers["status"] != "boss" && player.pers["status"] != "default") {
//	thread notifyAdmins(player.name,player.pers["status"]);
//	}
//}	
...
updateRankAnnounceHUD()
{
	...
	killz = self.pers["rankxp"];
	rankId = self getRankForXp(killz);
	self iprintlnbold(self volkv\_common::getLangString("KILLS_LEFT") + (getRankInfoMaxXp(rankId) - killz));
	
	if (killz < 300) {
	self iprintlnbold(self volkv\_common::getLangString("PREST_LEFT") + (300 - killz));	
	}
}
...
updateRankScoreHUD( amount )
{
	...
	{			
		...
		if ( self.rankUpdateTotal <= 0 )
		{
			...
			self.hud_rankscroreupdate.color = (1,0,1);
			
		} else if  ( self.rankUpdateTotal > 1000  )
		{
			self.hud_rankscroreupdate.label = &"MP_PLUS";
			self.hud_rankscroreupdate.color = (1,0,0);
		} else {
			self.hud_rankscroreupdate.label = &"MP_PLUS";
			red = self.rankUpdateTotal / 1000;
			green = 1 - red;
			self.hud_rankscroreupdate.color = (red,green,0);
		}
