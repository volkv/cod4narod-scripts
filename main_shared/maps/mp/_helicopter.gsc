...
heli_update_global_dvars()
{
    for ( ;; )
    {
        ...
        level.heli_loopmax = 10;			// how many times helicopter will circle the map before it leaves
        level.heli_missile_rof = 1;	// missile rate of fire, one every this many seconds per target, could fire two at the same time to different targets
        level.heli_armor = 100 + ( level.players.size * 30 );			// armor points, after this much damage is taken, helicopter is easily damaged, and health degrades
        level.heli_rage_missile = 50;	// higher the value, more frequent the missile assault
        level.heli_maxhealth = 500 + ( level.players.size * 50 );	// max health of the helicopter
        level.heli_missile_max = 30;	// max number of missiles helicopter can carry
        ...
        level.heli_missile_engage_dist = 5000;	// engaging distance for missiles
        level.heli_missile_regen_time = 5;			// gives one more missile to helicopter every interval - seconds
        ...
        level.heli_missile_friendlycare = 10;	// if friendly is within this distance of the target, do not shoot missile
        ...
        level.heli_armor_bulletdamage = 0.5;			// damage multiplier to bullets onto helicopter's armor
...
heliHUDinfo() {
    ...
    level.heliIcon[index] = newHudElem();
    level.heliIcon[index].x = -5;
    level.heliIcon[index].y = 20 + (index * 30);
    level.heliIcon[index].hidden = false;
    level.heliIcon[index].horzAlign = "right";
    level.heliIcon[index].vertAlign = "top";
    level.heliIcon[index].alignx = "right";
    level.heliIcon[index].alignY = "middle";
    level.heliIcon[index].alpha = 0;
    level.heliIcon[index].archived = false;
    level.heliIcon[index] fadeOverTime( .5 );
    level.heliIcon[index].alpha = .8;
    level.heliIcon[index] setShader( "compass_objpoint_helicopter", 32, 32 );

		level.heliIconTeam[index] = newHudElem();
    level.heliIconTeam[index].x = 0;
    level.heliIconTeam[index].y = 20 + (index * 30);
    level.heliIconTeam[index].hidden = false;
    level.heliIconTeam[index].horzAlign = "right";
    level.heliIconTeam[index].vertAlign = "top";
    level.heliIconTeam[index].alignx = "right";
    level.heliIconTeam[index].alignY = "middle";
    level.heliIconTeam[index].alpha = 0;
    level.heliIconTeam[index].archived = false;
    level.heliIconTeam[index] fadeOverTime( .5 );
    level.heliIconTeam[index].alpha = .8;
    level.heliIconTeam[index] setShader(  game["icons"][self.team], 14, 14 );
	
    level.heliHealth[index] = newHudelem();
    level.heliHealth[index].x = -30;
    level.heliHealth[index].y = 20 + (index * 30);
    level.heliHealth[index].horzAlign = "right";
    level.heliHealth[index].vertAlign = "top";
    level.heliHealth[index].alignX = "right";
    level.heliHealth[index].alignY = "middle";
    level.heliHealth[index].alpha = 0;
    level.heliHealth[index].color = (0,1,0);
	level.heliHealth[index] fadeOverTime( .5 );
    level.heliHealth[index] setText(level.heli_armor + self.maxhealth -self.damageTaken);
    level.heliHealth[index].fontScale = 1.4;
    level.heliHealth[index].alpha = .8;

    self thread heliHUDupdate(index);
}

heliHUDupdate(index) {

health = int(self.maxhealth - self.damageTaken);
maxhealth = self.maxhealth;

    while (isDefined(self.maxhealth) && (health) > 1) {

		health = int(self.maxhealth - self.damageTaken);
	
		green = health / maxhealth;
		red = 1 - green;
			
		level.heliHealth[index].color = (red,green,0);
        level.heliHealth[index] setText(health);
    ...
    level.heliHealth[index] destroy();
    level.heliIcon[index] destroy();
    level.heliIconTeam[index] destroy();

    level.heliHealth[index]= undefined;
    level.heliIcon[index] = undefined;
    level.heliIconTeam[index] = undefined;
...
heli_think( owner, startnode, heli_team, requiredDeathCount )
{
    ...
    chopper.evasive = true;							// evasive manuvering
    ...
    chopper.health_evasive = level.heli_maxhealth*0.5;			// when damage taken is above this value, helicopter performs evasive manuvering
...
heli_reset()
{
    self clearTargetYaw();
    self clearGoalYaw();
    self setspeed( 600, 70 );
    self setyawspeed( 150, 90, 90 );
    //self setjitterparams( (30, 30, 30), 4, 6 );
    self setmaxpitchroll( 50, 50 );
    ...
    for ( ;; )
    {
        ...
        if ( type == "MOD_RIFLE_BULLET" || type == "MOD_PISTOL_BULLET" )
        {
            if ( self.damageTaken >= self.health_bulletdamageble ) {
				...
				finaldamage = damage*level.heli_armor_bulletdamage;
                self.damageTaken += finaldamage;
			}
        }
        else {
            self.damageTaken += damage;
        ...
        if ( self.damageTaken > self.maxhealth ) {
			...
			break;
				...
				score = int((finaldamage / self.maxhealth) *500);
...
heli_health()
{
    ...
    for ( ;; )
    {
        ...
        if ( self.currentstate == "heavy smoke" )
        {
            ...
            level.heli_rage_missile = 100; // increase missile firing rate more
        }
        if ( self.currentstate == "light smoke" )
        {
            self.damageTaken += level.heli_health_degrade/2;
            level.heli_rage_missile = 80;	// increase missile firing rate
        }
...
attack_secondary()
{
    ...
    for ( ;; )
    {
        if ( isdefined( self.secondaryTarget ) && !level.event && game["state"] != "postgame")
        {
...
missile_support( target_player, rof, instantfire, endon_notify )
{
    ...
    if ( self.missile_ammo > 0 && isdefined( target_player ) && !level.event && game["state"] != "postgame" )
    {
...
fire_guided_missile( eTarget )
{
    ...
    PlayFX( level.hardEffects[ "tankerExp" ], collision[ "position" ] );
    radiusDamage( collision[ "position" ], 65, 120, 80, self, "MOD_PROJECTILE_SPLASH", "artillery_mp" );
...
attack_primary()
{
    ...
    for ( ;; )
    {
        if ( isdefined( self.primaryTarget ) && !level.event && game["state"] != "postgame")
        {
